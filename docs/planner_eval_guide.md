...
[
  ...
  },
  {
    "id": "T02",
    "type": "隐式-成药性",
    "user_request": "老板让我评估这条多肽作为口服药的潜力。",
    "ground_truth": ["toxicity_node", "stability_node"]
  },
  {
    "id": "T04",
    "type": "模糊/兜底",
...
5. **Hamming Loss (HL, 汉明损失)**
   ...
   - **含义**: 衡量“被错误预测的标签比例”。在有 5 个可用 Agent 的情况下，如果错了 1 个，对了 4 个，汉明损失就是 1/5 = 0.2。
...
================================================================================
数据集样本总数: 10
Exact Match Ratio (EMR, 严格命中率): 70.00% (7/10)
Micro-Precision: 90.91%
...
--------------------------------------------------------------------------------
❌ 错误路由样例分析 (Error Analysis):
[T02] 类型=隐式-成药性
    用户指令: 老板让我评估这条多肽作为口服药的潜力。
    Ground Truth: stability_node, toxicity_node
    Predicted   : activity_node, stability_node, toxicity_node
    [TP=2, FP=1, FN=0]
...
```
...
以下是基于当前 10 条黄金数据集（包含显式、隐式、兜底）使用大模型进行路由预测的最新基准结果：
...
================================================================================
数据集样本总数: 10
Exact Match Ratio (EMR, 严格命中率): 80.00% (8/10)
Hamming Loss (HL, 汉明损失)    : 0.0400 (越接近0越好)
Micro-Precision: 95.24%
Micro-Recall: 95.24%
Micro-F1 Score: 95.24%
--------------------------------------------------------------------------------
📊 细分类别表现 (按 Intent Type):
  [显式] 样本数: 3
    - EMR: 100.00% (3/3)
    - HL : 0.0000
    - F1 : 100.00% (P=100.00%, R=100.00%)
  [模糊/兜底] 样本数: 2
    - EMR: 100.00% (2/2)
    - HL : 0.0000
    - F1 : 100.00% (P=100.00%, R=100.00%)
...
--------------------------------------------------------------------------------
[T08] 类型=隐式-优化方向
    用户指令: 这条序列容易被酶降解，有什么改造建议？
    Ground Truth: esmfold_node, stability_node
    Predicted   : stability_node
    [TP=1, FP=0, FN=1]
--------------------------------------------------------------------------------
```

### 结果解读
- **整体稳定性强**：Micro-F1 高达 `95.24%`，且 Hamming Loss 仅为 `0.0400`，说明在所有的决策点中，系统犯错的“粒度”极小。
- **显式与兜底路由完美**：明确的指令以及模糊指令的兜底路由表现均达到 `100% EMR`。
- **隐式推理有待微调**：错误主要集中在“隐式意图”中，如评估成药性时多召唤了活性节点 (FP)，以及在给改造建议时漏掉了结构预测节点 (FN)。这可以通过后续补充 Few-shot 示例来进行优化。
