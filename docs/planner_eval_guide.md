...
5. **Hamming Loss (HL, 汉明损失)**
   ...
   - **含义**: 衡量“被错误预测的标签比例”。在有 5 个可用 Agent 的情况下，如果错了 1 个，对了 4 个，汉明损失就是 1/5 = 0.20。
...
================================================================================
数据集样本总数: 138
Exact Match Ratio (EMR, 严格命中率): 83.33% (115/138)
Micro-Precision: 87.45%
...
--------------------------------------------------------------------------------
...
以下是基于当前 138 条黄金数据集（包含显式、组合、隐式、兜底、困难压力样本，以及神经肽/论文模型相关样本）使用关键词兜底路由的最新基准结果。当前可选专家节点为 5 个：理化、毒性、活性、神经肽、结构。
...
数据集样本总数: 138
Exact Match Ratio (EMR, 严格命中率): 83.33% (115/138)
Hamming Loss (HL, 汉明损失)    : 0.0449 (越接近0越好)
Micro-Precision: 88.16%
Micro-Recall: 99.08%
Micro-F1 Score: 93.30%
--------------------------------------------------------------------------------
...
### 结果解读
- **基础路由仍然稳定**：常规显式、组合、隐式、兜底场景保持全部命中，说明新增神经肽节点没有影响原有主路径。
- **压力样本拉低了完美分数**：加入困难样本和论文模型相关样本后，EMR 为 `83.33%`，Micro-F1 为 `93.30%`，更接近真实系统评估。
